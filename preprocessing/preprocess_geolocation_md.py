...
def preprocess_geolocation_md():
    ...
    geolocations_df = pd.DataFrame(
        geolocations_tuples, columns=["loc1", "loc2", "loc3"]
    )

    # Join with accession_df and export to tsv file named geolocations+accessions.tsv

    accession_df.join(geolocations_df).to_csv(
        "/data/geolocations+accession.tsv", sep="\t"
    )


if __name__ == "__main__":
    preprocess_geolocation_md()